...
# currently we're dealing with dummy data so no article matches
for c in cancers:
    for d in drugs:
        #import pdb; pdb.set_trace()
        all = Article.query.filter(Article.cancer.ilike(c)).filter(Article.intervention.ilike(d))
        ...
        not_exists = db.session.query(Rank).filter(
                func.lower(Rank.cancer)==func.lower(c), func.lower(Rank.intervention)==func.lower(d)
            ).scalar() is None
        if not_exists:
            new_rank = Rank(
            all=all.count(),
